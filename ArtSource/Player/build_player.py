...
glass=mat('MAT_Display_Glass',(.004,.005,.007),.05,.32)
...
def soften(o):
    """Bake one subdivision level; weights interpolate with the cloth topology."""
    # Mirroring authored coordinates changes winding. Correct it before smoothing/export.
    bm=bmesh.new();bm.from_mesh(o.data)
    bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(o.data);bm.free()
    bpy.context.view_layer.objects.active=o
    mod=o.modifiers.new('Cloth surface','SUBSURF');mod.levels=1
    bpy.ops.object.modifier_apply(modifier=mod.name)
    for p in o.data.polygons:p.use_smooth=True
    return o

def piping(name,pts,r,m,bone,g='Outfit',cyclic=False):
    d=bpy.data.curves.new(name,'CURVE');d.dimensions='3D'
    d.bevel_depth=r;d.bevel_resolution=1;d.resolution_u=4
    sp=d.splines.new('POLY' if cyclic else 'BEZIER')
    if cyclic:
    ...
    return finish(bpy.context.object,name,m,bone,g,True)
...
hoodie=profile('Hoodie_Torso',[(1.019,.171,.109,0,0),(1.03,.182,.12,0,0),(1.057,.194,.133,0,0),(1.084,.190,.133,0,0),(1.12,.182,.128,0,0),(1.17,.181,.126,0,0),(1.23,.19,.127,0,0),(1.28,.201,.123,0,0),(1.32,.206,.114,0,0),(1.35,.175,.097,0,0),(1.384,.094,.074,0,0),(1.39,.082,.07,0,0)],cloth,None,'Outfit',24)
for v in hoodie.data.vertices:
    x,y,z=v.co;t=math.atan2(y/.13,x/.19)
    # Broad diagonal tension folds taper out before the shoulder and neckline.
    amount=.0045*math.sin(t*5+(z-1)*31)*math.exp(-((z-1.11)/.13)**2)
    v.co.x+=amount*math.cos(t);v.co.y+=amount*math.sin(t)
...
soften(hoodie)
soften(profile('RibHem',[(.994,.169,.110,0,0),(.998,.176,.116,0,0),(1.013,.178,.118,0,0),(1.030,.181,.119,0,0),(1.034,.179,.117,0,0)],cloth,'Spine','Outfit',32))
soften(profile('Hood_Drape',[(1.299,.099,.045,0,.119),(1.319,.131,.073,0,.119),(1.357,.158,.085,0,.11),(1.398,.153,.079,0,.105),(1.428,.123,.067,0,.094),(1.441,.099,.052,0,.082)],cloth,'Chest','Outfit',24))
# An open, folded collar with a V-shaped front, not a solid plug around the neck.
vs=[];fs=[];n=40
for j,(rx,ry,dz) in enumerate([(.067,.063,.009),(.076,.071,.020),(.095,.093,.025),(.134,.117,.004),(.132,.116,-.025),(.099,.085,-.035),(.067,.063,.009)]):
    for i in range(n):
        t=math.tau*i/n;front=max(0,-math.sin(t))
        vs.append((rx*math.cos(t),ry*math.sin(t)+.012,1.41+dz-.064*front**4))
for j in range(6):
    for i in range(n):a=j*n+i;b=j*n+(i+1)%n;fs.append((a,b,b+n,a+n))
soften(mesh('Hood_FoldedOpening',vs,fs,cloth,'Chest','Outfit'))
for s in [-1,1]:
    cyl('Cord_Eyelet',(s*.049,-.099,1.377),(s*.049,-.11,1.377),.008,steel,'Chest','Outfit',12)
    piping('Drawcord',[(s*.049,-.115,1.376),(s*.047,-.137,1.32),(s*.05,-.144,1.257),(s*.047,-.145,1.205-s*.008)],.0035,red,'Chest')
    cyl('Cord_Aglet',(s*.047,-.145,1.209-s*.008),(s*.047,-.145,1.187-s*.008),.0046,red,'Chest','Outfit',10)
soften(box('Kangaroo_Pocket',(0,-.133,1.094),(.239,.028,.094),cloth,'Spine','Outfit',.014,3))
for s in [-1,1]:beam('Pocket_Opening',(s*.113,-.153,1.078),(s*.083,-.154,1.135),.007,.004,pants,'Spine','Outfit',.002)
for side,s in [('Left',1),('Right',-1)]:
    rings=[(.12,.077,.075,.005,0),(.17,.091,.093,.001,0),(.22,.099,.103,0,0),(.265,.107,.105,-.005,0),(.31,.108,.102,-.012,0),(.36,.105,.098,-.014,0),(.405,.102,.097,-.014,0),(.44,.097,.095,-.013,0),(.472,.094,.093,-.010,0),(.495,.097,.094,-.012,0),(.52,.099,.095,-.015,0),(.545,.100,.095,-.019,0),(.572,.096,.091,-.021,0),(.597,.097,.089,-.019,0),(.621,.089,.082,-.015,0),(.645,.081,.076,-.012,0),(.661,.072,.067,-.008,0),(.675,.056,.054,-.003,0),(.683,.052,.052,0,0)]
    o=profile('Hoodie_Sleeve_'+side,rings,cloth,None,'Outfit',24)
    for v in o.data.vertices:
        x,y,z=v.co.copy();t=math.atan2(y,x)
        # Sparse diagonal folds at elbow and gathers above the narrow cuff.
        fold=.0055*math.sin((z-.44)*98+t*2.4)*math.exp(-((z-.49)/.065)**2)
        fold+=.0065*math.sin((z-.60)*125-t*2)*math.exp(-((z-.637)/.045)**2)
        fold+=.0018*math.sin(t*5+z*9)
        v.co=(s*z,y+fold*math.sin(t),1.34+x+fold*math.cos(t))
    for bn in ['Chest',side+'UpperArm',side+'LowerArm']:o.vertex_groups.new(name=bn)
    for v in o.data.vertices:
        w=max(0,min(1,(abs(v.co.x)-.427)/.126));w=w*w*(3-2*w)
        shoulder=max(0,min(1,(abs(v.co.x)-.135)/.19));shoulder=shoulder*shoulder*(3-2*shoulder)
    ...
    soften(o)
    cuff=profile('Ribbed_Cuff_'+side,[(.672,.052,.052,0,0),(.677,.057,.057,0,0),(.685,.056,.056,0,0),(.715,.053,.053,0,0),(.721,.052,.052,0,0),(.722,.048,.048,0,0)],cloth,side+'LowerArm','Outfit',64)
    for v in cuff.data.vertices:
        x,y,z=v.co.copy();t=math.atan2(y,x);f=1+.018*math.cos(t*32)
        v.co=(s*z,y*f,1.34+x*f)
    bm=bmesh.new();bm.from_mesh(cuff.data)
    bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(cuff.data);bm.free()
    for p in cuff.data.polygons:p.use_smooth=True
    for axis in [.68,.717]:
        piping('Cuff_Stitch',[(s*axis,.056*math.sin(math.tau*i/24),1.34+.056*math.cos(math.tau*i/24)) for i in range(24)],.0011,cloth,side+'LowerArm',cyclic=True)
    box('Sleeve_Tab',(s*.342,-.04,1.431),(.046,.025,.006),red,side+'UpperArm','Outfit',.003,3)
...
report['status']='PASS'
for s in [-1,1]:
    # Sleeve wall normals must face outward on both reflected arms in the rest mesh.
    tested=0
    for p in objects['Outfit'].data.polygons:
        c=p.center
        if .39<s*c.x<.56 and abs(c.y)<.13 and 1.08<c.z<1.32:
            radial=Vector((0,c.y,c.z-1.206))
            assert p.normal.dot(radial)>0,'Inverted sleeve surface'
            tested+=1
    assert tested>500,'Sleeve normal check did not cover its surface'
assert 'Face' not in objects,'Display must remain blank'
assert not bpy.data.materials.get('MAT_Display_Amber'),'No emissive face geometry'
report['display']='Blank black glass; no eyes or blink shape'
...
export('CHR_Player_Base',['Body','Chassis','Head'])
export('CHR_Player_Hoodie',['Body','Chassis','Head','Outfit','Gear'])
...
rig.animation_data.action=None
for side,s in [('Left',1),('Right',-1)]:
    p=rig.pose.bones[side+'LowerArm'];basis=p.bone.matrix_local.to_3x3().to_quaternion()
    p.rotation_euler=(basis.inverted() @ Quaternion((0,0,1),-s*math.radians(85)) @ basis).to_euler()
bpy.context.view_layer.update()
render('Player_Hoodie_ElbowCheck',(2.7,-5,2.3),True)
